...
/// ChatGPT's internal usage endpoint (see ClodexCore.CodexPlanUsage).
///
...
/// fetch (Codex refreshes it as it runs), never logged, and never sent anywhere
/// but chatgpt.com. CLODEX_CHATGPT_TOKEN overrides for tests.
///
/// Failure vocabulary: no_token / http_401 / network / bad_shape. A stale token
/// (401) self-heals the next time the user runs Codex.
...
final class CodexPlanFetcher: @unchecked Sendable {
    ...
    private static func accessToken() -> String? {
        if let env = ProcessInfo.processInfo.environment["CLODEX_CHATGPT_TOKEN"],
           !env.isEmpty {
            return env
        }
        let home: URL
