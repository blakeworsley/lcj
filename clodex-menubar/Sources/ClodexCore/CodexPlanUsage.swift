...
/// numbers; reset_at is epoch seconds. Undocumented internal endpoint — parse
/// defensively and degrade to the dollar-budget barometer when absent.
...
public struct CodexPlanUsage: Equatable, Sendable {
    ...
    public static func parse(_ body: Data) -> CodexPlanUsage? {
        ...
        if let p = limit["used_percent"] as? NSNumber {
            percent = max(0, min(100, Int(p.doubleValue.rounded())))
        } else if limitCredits > 0 {
            percent = max(0, min(100, Int((usedCredits / limitCredits * 100).rounded())))
        } else {
            percent = 0
        }
